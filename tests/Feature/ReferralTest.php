...
test('new users get a referral code generated automatically on registration', function () {
    ...
    expect($user->referral_code)->not->toBeNull()
        ...
        ->and(str_starts_with($user->referral_code, 'KOMP-'))->toBeTrue()
...
test('validates valid and invalid referral codes correctly', function () {
    $userA = User::factory()->create([
        ...
        'referral_code' => 'KOMP-USERAA',
        'point_balance' => 0
    ]);
    
    $userB = User::factory()->create([
        'email' => 'userb@example.com',
        'referral_code' => 'KOMP-USERBB',
        'point_balance' => 0
    ]);
    ...
    $response = $this->postJson('/api/referral/validate', ['code' => 'KOMP-USERAA']);
    $response->assertStatus(200)
        ->assertJson([
            'valid' => true,
            'referrer' => ['name' => $userA->name]
    ...
    $response = $this->postJson('/api/referral/validate', ['code' => 'KOMP-USERBB']);
    $response->assertStatus(200)
        ->assertJson([
            'valid' => false,
            'message' => 'Anda tidak bisa menggunakan kode referral Anda sendiri.'
...
test('referral reward point is awarded when invoice is paid', function () {
    ...
    $referrer = User::factory()->create(['referral_code' => 'KOMP-REFERR']);
