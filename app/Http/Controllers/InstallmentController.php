...
class InstallmentController extends Controller
{
    ...
    public function index()
    {
        ...
        return \Inertia\Inertia::render('user/profile/installments', [
    ...
    public function store(Request $request)
    {
        try {
            ...
            if (!$referredByUserId) {
                $defaultAffiliate = \App\Models\User::whereIn('affiliate_code', ['KMP2025', 'AKS2025'])->first()
                    ?? \App\Models\User::role('affiliate')->first();
            ...
            if (DB::getDriverName() === 'sqlite') {
                $parentCode = 'KMT-' . date('y') . str_pad((string) (Invoice::count() + 1), 5, '0', STR_PAD_LEFT);
            } else {
            ...
            // Buat Checkout Payment hanya untuk termin ke-1 (DP)
            $paymentUrl = $this->createCheckoutPayment($firstChildInvoice, $item, Auth::user());
    ...
    public function sendReminder(Request $request, string $id)
    {
        try {
            ...
            if (!empty($customMessage)) {
                ...
                $message = "*[Kompeten - Pengingat Pembayaran Cicilan]*\n\n";
    ...
    /**
     * Buat payment checkout menggunakan DokuService (atau fallback Xendit jika dikonfigurasi)
     */
    private function createCheckoutPayment(Invoice $childInvoice, mixed $item, mixed $user, ?string $orderNumber = null): string
    {
        $productTitle = $item?->title ?? 'Produk';
        $orderInvoiceCode = $orderNumber ?: $childInvoice->invoice_code;

        // 1. Coba DokuService jika tersedia
        try {
            ...
            if (!empty($dokuResponse['response']['payment']['url'])) {
                return $dokuResponse['response']['payment']['url'];
            }
        } catch (\Throwable $e) {
            Log::warning('Doku checkout for installment failed, checking Xendit fallback: ' . $e->getMessage());
        }

        // 2. Fallback Xendit jika Doku tidak mengembalikan URL
        if (config('xendit.API_KEY')) {
            try {
                \Xendit\Configuration::setXenditKey(config('xendit.API_KEY'));
                $xenditRequest = new \Xendit\Invoice\CreateInvoiceRequest([
                    'external_id' => $orderInvoiceCode,
                    'customer' => [
                    ...
                    'description' => 'Cicilan ke-' . $childInvoice->installment_number . ' untuk ' . $productTitle . ' - ' . $user->name,
                    'amount' => $childInvoice->amount,
                    'items' => [[
                        'name' => $productTitle . ' (Cicilan ke-' . $childInvoice->installment_number . ')',
                ...
                $xenditApi = new \Xendit\Invoice\InvoiceApi();
                $res = $xenditApi->createInvoice($xenditRequest)->getArrayCopy() ?? (array) $xenditApi->createInvoice($xenditRequest);
                if (!empty($res['invoice_url'])) {
                    return $res['invoice_url'];
                }
            } catch (\Throwable $e) {
                Log::error('Xendit fallback for installment failed: ' . $e->getMessage());
            }
        }

        throw new \Exception('Gagal membuat tautan pembayaran untuk cicilan ini.');
    }
}
